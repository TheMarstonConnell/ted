...
get **at least 48px control targets**, including sidebar actions and menu items.
...
then scrolls. Its bottom toolbar wraps model/effort controls independently from
...
focus stop, or click action. Model/effort changes use the composer toolbar. Dialogs scroll within the viewport and reserve title space for
the close control; destructive actions wrap on narrow screens.
...
- **Inter:** prose, headings, navigation, labels, buttons, and dialog usage data.
...
- `npm run test:e2e` checks workflow behavior plus measured grid constraints at
  ...
  settings screenshots. Existing tests cover queues, tools, images, drawer
