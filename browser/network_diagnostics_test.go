...
func TestNetworkDiagnostics(t *testing.T) {
	for _, test := range []struct {
		...
		{"tls", &network.EventLoadingFailed{RequestID: "tls", Type: network.ResourceTypeDocument, ErrorText: "net::ERR_CERT_AUTHORITY_INVALID"}, map[string]any{"text": "net::ERR_CERT_AUTHORITY_INVALID", "resource_type": network.ResourceTypeDocument}},
		{"dns", &network.EventLoadingFailed{RequestID: "dns", ErrorText: "net::ERR_NAME_NOT_RESOLVED"}, map[string]any{"text": "net::ERR_NAME_NOT_RESOLVED"}},
...
func TestNetworkHTTPDiagnostics(t *testing.T) {
	for _, status := range []int64{200, 302, 401, 403, 429, 503} {
		t.Run(fmt.Sprint(status), func(t *testing.T) {
			tab := &browserTab{}
			tab.handleNetworkDiagnostic(&network.EventResponseReceived{RequestID: "request", Type: network.ResourceTypeDocument, Response: &network.Response{
			...
			item := tab.errors[0].(map[string]any)
			if item["status"] != status || item["type"] != "http_error" || item["request_id"] != network.RequestID("request") {
				t.Fatalf("diagnostic = %#v", item)
			}
			encoded, _ := json.Marshal(item)
			if strings.Contains(string(encoded), "private-value") {
...
func TestNetworkDiagnosticsBoundedAndClearable(t *testing.T) {
	...
	for i := 0; i < 550; i++ {
		tab.handleNetworkDiagnostic(&network.EventLoadingFailed{RequestID: network.RequestID(fmt.Sprint(i))})
	}
	...
	if len(items) != 500 || items[0].(map[string]any)["request_id"] != network.RequestID("50") {
